...
need_stock_list = list() # 재무제표에서 조회해야 할 종목코드 들
...
for i,file_name in enumerate(file_list): # file_list 들에 대해서 반복
    ...
    need_stock_list += list(df['종목코드']) # pdf 파일의 column이 '종목코드'인 column을 list 화 한뒤 ,need_stock_list에 더함
    need_stock_list = list(set(need_stock_list)) #  # set()을 통해서 겹치는 종목코드를 제거하고 다시 리스트화
    
print()
print(len(need_stock_list)) # 만들어진 need_stock_list의 크기 출력
print(need_stock_list)# 만들어진 need_stock_list출력


with open("C:\self_project\snowball\Download_data\\need_stock_list.csv",'w', newline ='') as f:
    # using csv.writer method from CSV package
    write = csv.writer(f)
    for need_stock in need_stock_list:
        row=list() # 하나의 행에 저장을 할 list 초기화
        row.append(need_stock) # 리스트로 저장을 해야함 이렇게 안하면 문자열이 문자 하나하나 나누어 져서 저장이 됨
