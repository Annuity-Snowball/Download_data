...
class Backtest(Portfolio):
    ...
    def getProductTicker(self, sql_query, product_date):
        ...
        # print("in getProductTicker")
        # print(strategy_kind , product_count_per_strategy, min_value, max_value)
        ...
        elif strategy_kind == 'PER 고': # per고는 per 수치가 음수 인것들 중에서 절댓값이 가장 작은 것
            query = {"etf_date": product_date, "etf_per":{"$gt":0}}
            result = collection.find(query).sort("etf_per", -1).limit(product_count_per_strategy)
            
        elif strategy_kind == 'PER': # 수치는 양수만 입력 하세요
        ...
        elif strategy_kind == 'PBR': # 수치는 양수만 입력 하세요
        ...
        elif strategy_kind == 'OperatingRatio': # 수치는 양수만 입력 하세요
        ...
        elif strategy_kind == 'ProfitRatio': # 수치는 양수만 입력 하세요
        ...
        elif strategy_kind == 'DebtRatio': # 수치는 양수만 입력 하세요
        ...
        elif strategy_kind == 'ROE': # 수치는 양수만 입력 하세요
        ...
        # print(strategy_kind, product_ticker_list)
    ...
    def doBackTest(self):  
        ...
        return self.portfolio_object.portfolio_account_with_tax_benefit['총 수익률']

 
 # 정해진 기간에서 랜덤으로 날짜를 선택한뒤, 선택한 날짜와 365일뒤 날짜를 한 쌍으로 반환하는 함수

random.seed(42)
def random_year_dates(start_date, end_date, year_date): 
    start_datetime = datetime.strptime(start_date, '%Y-%m-%d')
    end_datetime = datetime.strptime(end_date, '%Y-%m-%d')
    random_date = start_datetime + timedelta(days=random.randint(0, (end_datetime - start_datetime).days))
    future_date = random_date + timedelta(days=year_date)
    return random_date.strftime('%Y-%m-%d'), future_date.strftime('%Y-%m-%d')

# 입력받은 리스트의 평균값을 반환하는 함수
def calculate_average(lst):
    total = sum(lst)
    length = len(lst)
    if length == 0:
        return None  # 빈 리스트의 경우 None을 반환하거나 예외를 발생시킬 수 있습니다.
    else:
        return total / length
...
if __name__ == "__main__":
    ...
    random_1year_date_list = list()
    random_2year_date_list = list()
    random_3year_date_list = list()
    
    
    for _ in range(10):
        random_pair = random_year_dates("2016-01-04", "2021-12-29",365)
        random_1year_date_list.append(random_pair)
        
        # random_pair = random_year_dates("2016-01-04", "2020-12-29",730)
        # random_2year_date_list.append(random_pair)
        
        # random_pair = random_year_dates("2016-01-04", "2019-12-29",1095)
        # random_3year_date_list.append(random_pair)
        
    
    # 함수 호출
    strategy_kind = ['PER 저','PER 고','PBR 저','PBR 고','PSR 저','PSR 고','OperatingRatio 저','OperatingRatio 고','ProfitRatio 저','ProfitRatio 고','DebtRatio 저','DebtRatio 고','ROE 저','ROE 고']

    stragy_ratio = [(5,95),(10,90),(15,85),(20,80),(25,75),(30,70),(35,65),(40,60),(45,55),(50,50),(55,45),(60,40),(65,35),(70,30),(75,25),(80,20),(85,15),(90,10),(95,5)]

    strategy_count = [(1,1),(1,2),(1,3),(2,1),(2,2),(2,3),(3,1),(3,2),(3,3)]

    rebalance_list = [3,6,12]
    
    # 빈 데이터 프레임 생성
    # 컬럼 이름 정의
    columns = ['전략1', '전략1비율', '전략1개수', '전략2', '전략2비율', '전략2개수', '리밸런싱', '투자기간', '결과']

    # 데이터가 없는 데이터프레임 생성
    df = pd.DataFrame(columns=columns)
    df_index = 0

    for i in range(len(strategy_kind)-1): 
        for j in range(i+1,len(strategy_kind)): 
            for first_strategy_ratio, second_strategy_ratio in stragy_ratio:
                for first_strategy_product_count, second_strategy_product_count in strategy_count:
                    for rebalance_duration in rebalance_list:
                        
                        # 1년 수익률들 리스트 초기화
                        backtest_result_list = list()
                        print('전략1 :',strategy_kind[i], ', 전략1 비율 :',first_strategy_ratio, ', 전략1 금융상품 개수 :',first_strategy_product_count, 
                                '/','전략2 :',strategy_kind[j], ', 전략2 비율 :',second_strategy_ratio, ', 전략2 금융상품 개수 :',second_strategy_product_count, 
                                '/','리밸런싱 주기 :',rebalance_duration,
                                '/','투자기간 :',1)
                        
                        for start_date, end_date in random_1year_date_list:
                            # 시작금액, 납입금액, 전략 종류(고,저) 고정 -> 객체 생성
                            backtest_object = Backtest('189', 'test', 1000000, [first_strategy_ratio, second_strategy_ratio], 
                                                       start_date, end_date, rebalance_duration, '0', 500000, 
                                                       [[strategy_kind[i], first_strategy_product_count, 0, 0], [strategy_kind[j], second_strategy_product_count, 0, 0]])
                            # 시뮬레이션 코드 결과값 1년 수익률들 리스트에 추가
                            result = backtest_object.doBackTest()
                            print('시작날짜 :',start_date, ', 끝날짜 :',end_date, ', 수익률 :',result)
                            backtest_result_list.append(result)
                            
                        # 1년 수익률 평균을 계산
                        result_average = calculate_average(backtest_result_list)
                        print('-------')
                        print('평균 수익률 :', result_average)
                        print('-------')
                        # 기계학습 데이터프레임에 값 추가
                        df.loc[df_index,:] = [strategy_kind[i], first_strategy_ratio, first_strategy_product_count,
                                              strategy_kind[j], second_strategy_ratio, second_strategy_product_count,
                                              rebalance_duration, 1, result_average]
                        df_index +=1
                            
                        # for start_date, end_date in random_2year_date_list:
                        #     print('전략1 :',strategy_kind[i], ', 전략1 비율 :',first_strategy_ratio, ', 전략1 금융상품 개수 :',first_strategy_product_count, 
                        #         '/','전략2 :',strategy_kind[j], ', 전략2 비율 :',second_strategy_ratio, ', 전략2 금융상품 개수 :',second_strategy_product_count, 
                        #         '/','리밸런싱 주기 :',rebalance_duration,
                        #         '/','시작날짜 :',start_date, ', 끝날짜 :',end_date,
                        #         '/','투자기간 :',2)
                            
                        # for start_date, end_date in random_3year_date_list:
                        #     print('전략1 :',strategy_kind[i], ', 전략1 비율 :',first_strategy_ratio, ', 전략1 금융상품 개수 :',first_strategy_product_count, 
                        #         '/','전략2 :',strategy_kind[j], ', 전략2 비율 :',second_strategy_ratio, ', 전략2 금융상품 개수 :',second_strategy_product_count, 
                        #         '/','리밸런싱 주기 :',rebalance_duration,
                        #         '/','시작날짜 :',start_date, ', 끝날짜 :',end_date,
