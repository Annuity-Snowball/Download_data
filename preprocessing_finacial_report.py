...
for null_category in null_dict: # null_dict의 key값들에 대해서 반복
    ...
    for null_stock_code in null_dict[null_category]: # 'ifrs_CurrentAssets' 에서 결측치를 가진 '종목코드'들을 추출
        for category_name in category_name_list:
            ...
            if len(df_temp) >0:
                df_2016_2Q_BS.loc[null_stock_code][null_category] = df_temp.loc[df_temp.index[0]]['당기 반기말']
            
# 위의 for 문으로 결측치 채우지 못하는 경우 직접 대입            
df_base[df_base['종목코드']=='036260']
...
df_2016_2Q_BS.columns = ['유동자산', '자산총계', '유동부채', '부채총계','결산기준일'] # 컬럼명들을 수정
df_2016_2Q_BS = df_2016_2Q_BS.reset_index('종목코드') # 인덱스를 컬럼으로 초기화
df_2016_2Q_BS
df_2016_2Q_BS.to_csv('C:\\Users\\LG\\Desktop\\result\\2016_2Q_BS.csv', index=False,encoding='cp949') # 파일로 저장
'''

# 2016년 3분기 재무상태표저장
'''
file = open("C:\\Users\\LG\\Desktop\\financial_report\\2016_3Q_BS.txt", "r")
...
# 결과 데이터프레임 생성 = 틀 생성
df_2016_3Q_BS = pd.DataFrame({},columns=column_list, index=df_basic['종목코드'].unique())
df_2016_3Q_BS.index.name = '종목코드' # 데이터프레임의 인덱스명을 설정

# 결과 데이터프레임의 에 맞는 값들을 할당하는 함수
def func(df):
    df_2016_3Q_BS.loc[df['종목코드']][df['항목코드']]=df['당기 3분기말']
    df_2016_3Q_BS.loc[df['종목코드']]['결산기준일']=df['결산기준일']
df_basic.apply(func,axis=1) # 함수 적용


# 결과데이터프레임 결측치확인
check_null_frame = df_2016_3Q_BS[df_2016_3Q_BS['ifrs_CurrentAssets'].isnull() | df_2016_3Q_BS['ifrs_Assets'].isnull() | df_2016_3Q_BS['ifrs_CurrentLiabilities'].isnull() | df_2016_3Q_BS['ifrs_Liabilities'].isnull()].copy()
# print(null_stock_code)
...
for null_category in null_dict: # null_dict의 key값들에 대해서 반복
    ...
    for null_stock_code in null_dict[null_category]: # 'ifrs_CurrentAssets' 에서 결측치를 가진 '종목코드'들을 추출
        for category_name in category_name_list:
            ...
            df_temp = df_temp[df_temp['항목명'].str.strip() == category_name] # '항목명'이 category_name인 것을 선택
            if len(df_temp) >0:
                df_2016_3Q_BS.loc[null_stock_code][null_category] = df_temp.loc[df_temp.index[0]]['당기 3분기말']
...
df_2016_3Q_BS.columns = ['유동자산', '자산총계', '유동부채', '부채총계','결산기준일'] # 컬럼명들을 수정
df_2016_3Q_BS = df_2016_3Q_BS.reset_index('종목코드') # 인덱스를 컬럼으로 초기화
df_2016_3Q_BS.to_csv('C:\\Users\\LG\\Desktop\\result\\2016_3Q_BS.csv', index=False,encoding='cp949') # 파일로 저장
'''

# 2016년 4분기 재무상태표저장
'''
file = open("C:\\Users\\LG\\Desktop\\financial_report\\2016_4Q_BS.txt", "r")
...
# 결과 데이터프레임 생성 = 틀 생성
df_2016_4Q_BS = pd.DataFrame({},columns=column_list, index=df_basic['종목코드'].unique())
df_2016_4Q_BS.index.name = '종목코드' # 데이터프레임의 인덱스명을 설정

# 결과 데이터프레임의 에 맞는 값들을 할당하는 함수
def func(df):
    df_2016_4Q_BS.loc[df['종목코드']][df['항목코드']]=df['당기']
    df_2016_4Q_BS.loc[df['종목코드']]['결산기준일']=df['결산기준일']
df_basic.apply(func,axis=1) # 함수 적용


# 결과데이터프레임 결측치확인
check_null_frame = df_2016_4Q_BS[df_2016_4Q_BS['ifrs_CurrentAssets'].isnull() | df_2016_4Q_BS['ifrs_Assets'].isnull() | df_2016_4Q_BS['ifrs_CurrentLiabilities'].isnull() | df_2016_4Q_BS['ifrs_Liabilities'].isnull()].copy()
# print(null_stock_code)
...
for null_category in null_dict: # null_dict의 key값들에 대해서 반복
    ...
    for null_stock_code in null_dict[null_category]: # 'ifrs_CurrentAssets' 에서 결측치를 가진 '종목코드'들을 추출
        for category_name in category_name_list:
            ...
            df_temp = df_temp[df_temp['항목명'].str.strip() == category_name] # '항목명'이 category_name인 것을 선택
            if len(df_temp) >0:
                df_2016_4Q_BS.loc[null_stock_code][null_category] = df_temp.loc[df_temp.index[0]]['당기']
...
df_2016_4Q_BS.columns = ['유동자산', '자산총계', '유동부채', '부채총계','결산기준일'] # 컬럼명들을 수정
df_2016_4Q_BS = df_2016_4Q_BS.reset_index('종목코드') # 인덱스를 컬럼으로 초기화
df_2016_4Q_BS.to_csv('C:\\Users\\LG\\Desktop\\result\\2016_4Q_BS.csv', index=False,encoding='cp949') # 파일로 저장
'''
...
for bs_info in bs_info_list:
    file = open("C:\\Users\\LG\\Desktop\\financial_report\\"+bs_info[0], "r")
    ...
    # 결과데이터프레임 결측치확인
    check_null_frame = df_BS[df_BS['ifrs_CurrentAssets'].isnull() | df_BS['ifrs_Assets'].isnull() | df_BS['ifrs_CurrentLiabilities'].isnull() | df_BS['ifrs_Liabilities'].isnull()].copy()
    # print(null_stock_code)
    ...
    df_BS.to_csv('C:\\Users\\LG\\Desktop\\result\\'+bs_info[2], index=False,encoding='cp949') # 파일로 저장
